...
export const AlertModal: React.FC<{ state: AlertState; onClose: () => void }> = ({ state, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4 animate-fadeIn" onClick={onClose}>
            <div className="bg-white dark:bg-[#3e3535] p-6 rounded-lg max-w-sm w-full shadow-xl border border-red-100 dark:border-[#4a4040]" onClick={e => e.stopPropagation()}>
                <h3 className="text-lg font-bold text-red-600 dark:text-red-400 mb-2">{state.title}</h3>
                <p className="text-gray-700 dark:text-gray-300 mb-4">{state.message}</p>
                <button onClick={onClose} className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-4 rounded transition">
                    OK
                </button>
            </div>
        </div>
